...
void ofApp::setup() {
	...
	for (int i=0; i<nTri; i++) {	//Scan all the triangles
		...
		center *= sphereRadius;	//Now the center vector has
		//length Rad
		...
		for (int j=0; j<3; j++) {
			vertices[ i*3 + j ] =
				center + ofPoint( ofRandom( -triangleSize, triangleSize ),
				ofRandom( -triangleSize, triangleSize ),
				ofRandom( -triangleSize, triangleSize ) );
...
void ofApp::draw(){
    ...
    readings[index] = ofMap(beat.kick(), 0, 1, 0.8, 1.4);
    cout << "reading: " << readings[index] << endl;
    total = total + readings[index];
    index = index + 1;
    ...
    ofScale(average, average, 1); // scale
    
    // 1
    //float scaleFactor = ofMap(mouseX, 0, ofGetWidth(), 0, 3);
    ...
    // 3
    // mouse control
    ///float mouseAngle = ofMap(mouseX, 0, ofGetWidth(), 0, 360);
    //ofRotate(mouseAngle, 0, 1, 0);
